...
class PaymentsDAOTest {
    ...
    void testDeletePayment() {
        try {
            ...
            assertThrows(PaymentNotFoundException.class, () -> paymentsDAO.readPayment(payment.getPaymentID()));
        } catch (DatabaseOperationException e) {
            fail(e.getMessage(), e);
        }
    }
    ...
    void testReadAllPaymentsForRental() {
        try {
            // Create payment records for a specific rental or ensure there are existing payment records in the database

            // Read all payment records for the rental
            List<PaymentsDTO> paymentRecords = paymentsDAO.readAllPaymentsForRental(payment.getRentalID());
