...
public class MaintenanceDAO {
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
      ...
      try (ResultSet resultSet = preparedStatement.executeQuery()) {
        if (resultSet.next()) {
          ...
          throw new MaintenanceNotFoundException("Maintenance not found with ID: " + maintenanceID);
        }
      }
    } catch (SQLException e) {
      throw new DatabaseOperationException("Error while reading maintenance: " + maintenanceID, e);
    }
  }
  ...
  public void deleteMaintenance(int maintenanceID) throws DatabaseOperationException {
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
      ...
      throw new DatabaseOperationException("Error while deleting maintenace: " + maintenanceID, e);
    }
  }
